...
parser.add_argument('out', type=argparse.FileType('w', encoding='UTF-8'), help='Output file with %%{filename}%% macros.')
...
for line in args.file.readlines():
    ...
    with open(import_file) as f2:
        ...
        for l2 in f2.readlines():
            ...
            if l2_out:
                print('%s%s' % (l2_out, args.mode == 'json' and ',' or ''), file=args.out)
            l2 = l2.rstrip('\n\r')
            l2_out = '%s%s%s' % (prefix, l2.replace('"', '\\"'), suffix)
        if l2_out:
            print(l2_out, file=args.out)